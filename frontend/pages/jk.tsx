...
import { getValidState as getNodeStateToQuerify, state2query } from "@/utils"
...
export default function JKPage() {
    ...
    const nodeState: NodeState<BaseNodeSpec> = useSelector((state: RootState) => state.node)
    const nodeStateToQuerify = getNodeStateToQuerify(nodeState)
    ...
    const doAnalyze: ButtonProps["onClick"] = async (event) => {
        const memReq: APISaveToMongo.RequestType = {
            query: state2query(nodeStateToQuerify, " ", "pr_mem_id"),
        }
    ...
    const dataSource: TableDataType[] = Object.keys(nodeState).map((tableName) => ({
        table: tableName,
